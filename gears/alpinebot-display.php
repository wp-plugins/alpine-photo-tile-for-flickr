  ...
  function parse_php_serial_v2($_flickr_php){
    ...
    if( !empty($content) && !empty($photos) && is_array( $photos ) ){
      ...
      foreach( $photos as $info ){
        ...
        $the_photo['image_caption'] = @esc_attr( $the_photo['image_caption']  ); // Encodes <, >, &, " and ' characters
        $the_photo['image_caption'] = @str_replace('"','',@str_replace("'",'',$the_photo['image_caption']));  // Not necessary, but just to be safe.
...
<script>
  ...
  if( jQuery.isFunction( jQuery(window).on ) ){
    jQuery(window).on('load', alpineOnLoad );
  }else if( jQuery.isFunction( jQuery(window).bind ) ){
    jQuery(window).bind('load', alpineOnLoad );
  }else{
    jQuery(window).bind('load', alpineOnLoad );
  }
</script>");  
